...
public class ServerLockActor extends EventSourcedBehavior<LockCommand, LockEvent, BusinessLockState> {
	...
	public ServerLockActor(
		...
		super(PersistenceId.of("ClusterLock", server, "-"));
		this.context = context;
		this.registeSelf(server);
		context.getLog().info("Start Lock Context:{}", server);
	}
	...
	public static ServiceKey<LockCommand> serviceKey(
			String server) {
		return ServiceKey.create(
				LockCommand.class, 
				server + ":ServerLockActor");
	}
